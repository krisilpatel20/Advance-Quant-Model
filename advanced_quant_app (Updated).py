...
# ==========================================
# TAB 17: CVD & VOLUME DELTA — SAFE MODE
# ==========================================
try:
    with tab17:
        st.header('📊 CVD & Volume Delta')
        st.caption('Safe mode: this tab does nothing heavy until you turn it on.')
        load_cvd = st.checkbox('Load CVD module', value=False, key='safe_load_cvd_v1')
        if not load_cvd:
            st.info('CVD tab is ready. Turn it on when needed.')
            ...
            delta = vol * np.sign(d['Close'].diff().fillna(0))
            cvd = delta.cumsum()
            cvd_ma = cvd.ewm(span=8, adjust=False).mean()
            ...
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06, subplot_titles=(f'{TICKER} Price', 'CVD'))
            fig.add_trace(go.Scatter(x=d.index, y=d['Close'], mode='lines', name='Close'), row=1, col=1)
            fig.add_trace(go.Scatter(x=d.index, y=cvd, mode='lines', name='CVD'), row=2, col=1)
            fig.add_trace(go.Scatter(x=d.index, y=cvd_ma, mode='lines', name='CVD EMA'), row=2, col=1)
            fig.update_layout(height=650, template='plotly_dark', hovermode='x unified')
            st.plotly_chart(fig, use_container_width=True)
except Exception as e:
    _safe_last_tab_error(tab17, 'CVD tab', e)


# ==========================================
# TAB 18: INSTITUTIONAL VWAP — SAFE MODE
# ==========================================
try:
    with tab18:
        st.header('📈 Institutional VWAP')
        st.caption('Safe VWAP tab. Uses current loaded OHLCV only.')
        load_vwap = st.checkbox('Load VWAP module', value=False, key='safe_load_vwap_v1')
        if not load_vwap:
            st.info('VWAP tab is ready. Turn it on when needed.')
        elif df_main is None or df_main.empty:
            st.warning('Please load a ticker first.')
        else:
            d = df_main.copy()
            ...
            vol = d['Volume'] if 'Volume' in d.columns else pd.Series(np.ones(len(d)), index=d.index)
            typical = (high + low + d['Close']) / 3.0
            vwap = (typical * vol).cumsum() / vol.cumsum().replace(0, np.nan)
            dev = (d['Close'] / vwap - 1.0) * 100
            c1, c2, c3 = st.columns(3)
            c1.metric('Close vs VWAP', f"{dev.iloc[-1]:+.2f}%")
            c2.metric('VWAP', f"{vwap.iloc[-1]:.2f}")
            c3.metric('Bias', 'Above VWAP' if d['Close'].iloc[-1] > vwap.iloc[-1] else 'Below VWAP')
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=d.index, y=d['Close'], mode='lines', name='Close'))
            fig.add_trace(go.Scatter(x=d.index, y=vwap, mode='lines', name='VWAP'))
            fig.update_layout(height=520, template='plotly_dark', hovermode='x unified', title=f'{TICKER} Close vs VWAP')
            st.plotly_chart(fig, use_container_width=True)
except Exception as e:
    _safe_last_tab_error(tab18, 'VWAP tab', e)


# ==========================================
# TAB 19: TIME SERIES ANALYSIS — SAFE MODE
# ==========================================
try:
    with tab19:
        st.header('🔬 Time Series Analysis')
        st.caption('Safe mode: returns, rolling volatility, and autocorrelation only after loading.')
        load_ts = st.checkbox('Load Time Series module', value=False, key='safe_load_ts_v1')
        if not load_ts:
            st.info('Time Series tab is ready. Turn it on when needed.')
        elif df_main is None or df_main.empty:
            st.warning('Please load a ticker first.')
        else:
            d = df_main.copy()
            ...
            ac1 = rets.autocorr(lag=1) if len(rets) > 5 else np.nan
            c1, c2, c3 = st.columns(3)
            c1.metric('Mean Daily Return', f"{rets.mean()*100:.3f}%")
            c2.metric('20-Bar Ann. Vol', f"{roll_vol.iloc[-1]:.2f}%" if pd.notna(roll_vol.iloc[-1]) else 'N/A')
            c3.metric('Lag-1 Autocorr', f"{ac1:.3f}" if pd.notna(ac1) else 'N/A')
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06, subplot_titles=('Returns', 'Rolling Volatility'))
            fig.add_trace(go.Scatter(x=rets.index, y=rets * 100, mode='lines', name='Return %'), row=1, col=1)
            fig.add_trace(go.Scatter(x=roll_vol.index, y=roll_vol, mode='lines', name='20-Bar Ann Vol %'), row=2, col=1)
            fig.update_layout(height=650, template='plotly_dark', hovermode='x unified')
            st.plotly_chart(fig, use_container_width=True)
except Exception as e:
    _safe_last_tab_error(tab19, 'Time Series tab', e)


# ==========================================
# TAB 20: MARKET MICROSTRUCTURE — SAFE MODE
# ==========================================
try:
    with tab20:
        st.header('🧬 Market Microstructure')
        st.caption('Safe proxy module. No Databento/network calls on app load.')
        load_micro = st.checkbox('Load Microstructure proxy module', value=False, key='safe_load_micro_v1')
        if not load_micro:
            st.info('Microstructure tab is ready. Turn it on when needed.')
        elif df_main is None or df_main.empty:
            st.warning('Please load a ticker first.')
        else:
            d = df_main.copy()
            ...
            high = d['High'].astype(float) if 'High' in d.columns else close
            low = d['Low'].astype(float) if 'Low' in d.columns else close
            vol = d['Volume'].astype(float) if 'Volume' in d.columns else pd.Series(np.ones(len(d)), index=d.index)
            rng = (high - low).replace(0, np.nan)
            ofi_proxy = (((close - low) / rng) - 0.5).fillna(0) * vol
            impact_proxy = close.pct_change().abs() / (vol.replace(0, np.nan) / vol.rolling(20, min_periods=5).mean()).replace(0, np.nan)
            impact_proxy = impact_proxy.replace([np.inf, -np.inf], np.nan).fillna(0)
            c1, c2, c3 = st.columns(3)
            c1.metric('Latest OFI Proxy', f"{ofi_proxy.iloc[-1]:+,.0f}")
            c2.metric('Impact Proxy', f"{impact_proxy.iloc[-1]:.5f}")
            c3.metric('Micro Bias', 'Buyer pressure' if ofi_proxy.tail(5).sum() > 0 else 'Seller pressure')
            fig = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.06, subplot_titles=('Close', 'Order-Flow Imbalance Proxy'))
            fig.add_trace(go.Scatter(x=d.index, y=close, mode='lines', name='Close'), row=1, col=1)
            fig.add_trace(go.Bar(x=d.index, y=ofi_proxy, name='OFI Proxy'), row=2, col=1)
            fig.update_layout(height=650, template='plotly_dark', hovermode='x unified')
            st.plotly_chart(fig, use_container_width=True)
except Exception as e:
    _safe_last_tab_error(tab20, 'Microstructure tab', e)
