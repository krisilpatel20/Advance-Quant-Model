...
def walk_forward_regime_selection(prices, returns, n_regimes=2, switch_vol=True, switch_trend=True, train_window=126, forward_window=21, conviction=0.65, min_hold=3, initial_capital=10000.0, trailing_stop_pct=0.0, stop_loss_pct=0.0, confirmed_bar=True, use_strong_runner_override=True, activity_mode="Conservative"):
    ...
    while start < len(idx):
        ...
        if chosen_method == "Strong Runner Trend Hold":
            test_signal_base = strong_runner_trend_hold_signal(combo_px).reindex(test_idx).ffill().fillna(0).clip(0, 1)
            if confirmed_bar:
                test_signal_base = test_signal_base.shift(1).ffill().fillna(float(chosen["signal"].iloc[-1]) if len(chosen["signal"]) else 0.0).clip(0, 1)
        else:
            latest_exposure = float(chosen["signal"].iloc[-1]) if len(chosen["signal"]) else 0.0
            test_signal_base = pd.Series(latest_exposure, index=test_idx, dtype=float)

        # This is the key fix: Conservative/Balanced/Active now changes the actual
        # out-of-sample forward signal, so returns and trade counts can differ.
        test_signal = apply_regime_activity_to_forward_signal(
            test_signal_base, combo_px, test_idx,
            mode=activity_mode,
            confirmed_bar=bool(confirmed_bar)
        )
...
def regime_activity_pulse_signal(prices, mode="Balanced"):
    """
    Causal price-action overlay used only to control Regime WFO trade activity.

    Conservative: no overlay is applied elsewhere.
    Balanced: moderate EMA/momentum pulse.
    Active: faster EMA/momentum pulse.

    This is intentionally simple: it creates real forward-bar entries/exits so the
    activity setting changes the actual trade log, not just the label or score.
    """
    px = pd.Series(prices).replace([np.inf, -np.inf], np.nan).dropna()
    if len(px) < 15:
        return pd.Series(0.0, index=px.index)
    ...
    if mode == "active":
        fast, slow, exit_span, mom_len, confirm = 5, 13, 21, 3, 1
...
def apply_regime_activity_to_forward_signal(base_signal, prices_window, test_idx, mode="Conservative", confirmed_bar=True):
    """
    Makes Conservative/Balanced/Active materially different in Regime WFO.
    - Conservative: uses the original WFO signal.
    - Balanced: blends original WFO with a moderate trend pulse.
    - Active: uses a faster trend pulse, allowing more entries/exits.
    """
    mode_clean = str(mode or "Conservative")
    base = pd.Series(base_signal, index=test_idx, dtype=float).ffill().fillna(0.0).clip(0, 1)
    if mode_clean == "Conservative":
        return base

    pulse_full = regime_activity_pulse_signal(prices_window, mode=mode_clean)
    pulse = pulse_full.reindex(test_idx).ffill().fillna(float(base.iloc[0]) if len(base) else 0.0).clip(0, 1)
    if confirmed_bar:
        pulse = pulse.shift(1).ffill().fillna(float(base.iloc[0]) if len(base) else 0.0).clip(0, 1)

    if mode_clean == "Balanced":
        # Balanced: allow trend pulse to enter/exit, but require either original WFO support
        # or a confirmed trend pulse. This creates more trades without becoming fully noisy.
        combined = (((base >= 0.5) & (pulse >= 0.5)) | ((base < 0.5) & (pulse >= 0.5))).astype(float)
        return pd.Series(combined, index=test_idx).ffill().fillna(0.0).clip(0, 1)

    # Active: faster pulse directly controls exposure inside the WFO forward block.
    return pulse
...
with tab7:
    ...
    if strategy_type == "Regime Switching (Trend Following)":
        ...
        with st.expander("🧭 Regime WFO Settings", expanded=True):
            ...
            regime_wfo_activity_mode = st.radio(
                "Regime WFO activity",
                ["Conservative", "Balanced", "Active"],
                index=1,
                horizontal=True,
                key="bt_regime_wfo_activity_mode",
                help="Conservative keeps original block-level WFO. Balanced adds a moderate causal trend pulse. Active uses a faster trend pulse for more entries/exits."
            )
                    ...
                    if enable_regime_wfo:
                        with st.spinner("Running Regime Walk-Forward Optimization..."):
                            wf_regime = walk_forward_regime_selection(
                                ...
                                activity_mode=str(regime_wfo_activity_mode)
                            )
