...
# ==========================================
# MARKET MICROSTRUCTURE TAB — SAFE LAZY PLACEHOLDER
# ==========================================
# This keeps the full app/model loading normally. The previous merged
# microstructure implementation was too heavy and could block Streamlit.
# Nothing here runs Databento or OHLCV backtests automatically.
try:
    with tab20:
        st.header("🧬 Market Microstructure")
        st.warning(
            "Market Microstructure heavy engine is temporarily disabled so the main model loads normally. "
            "This tab is now safe/lazy and will not run Databento, replay, charts, or backtests on app load."
        )
        st.markdown("""
        **Current safe mode**
        - Main app should load normally.
        - Regime Switching, IV Proxy, VWAP/CVD, and other tabs are not touched.
        - Databento is not imported or connected here.
        - No microstructure backtest runs automatically.

        Next clean step should be adding microstructure back as a separate button-only module, one small piece at a time.
        """)
except Exception as e:
    try:
        with tab20:
            st.error(f"Market Microstructure safe placeholder error: {e}")
    except Exception:
        pass
