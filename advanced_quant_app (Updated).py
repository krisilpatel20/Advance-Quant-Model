...
def _raw_session_close_for_display_date(raw_prices, dt_value):
    """
    DISPLAY ONLY.
    Return raw Close for the same displayed session date. This never changes signals,
    model fitting, equity curve, stops, or strategy selection.
    """
    try:
        if raw_prices is None:
            return np.nan
        s = str(dt_value).replace(" CT", "").replace(" CST", "").replace(" CDT", "").strip()
        ...
        if isinstance(raw_prices, pd.DataFrame):
            if "Close" not in raw_prices.columns:
                return np.nan
            px = raw_prices["Close"].copy()
        else:
            px = pd.Series(raw_prices).copy()
        ...
        try:
            ...
            pass
        px = pd.Series(px.values, index=idx)
        same_day = px[pd.DatetimeIndex(px.index).date == day]
        if same_day.empty:
            return np.nan
...
def _raw_session_high_low_for_display_date(raw_ohlc, dt_value):
    """
    DISPLAY ONLY.
    Return raw session low/high for validating whether a displayed fill was tradable.
    """
    try:
        ...
        try:
            ...
            pass
        df.index = idx
        df = df[pd.DatetimeIndex(df.index).date == day]
        if df.empty:
            return np.nan, np.nan
        low_col = "Low" if "Low" in df.columns else ("Close" if "Close" in df.columns else None)
        high_col = "High" if "High" in df.columns else ("Close" if "Close" in df.columns else None)
        if low_col is None or high_col is None:
            return np.nan, np.nan
        lo = pd.to_numeric(df[low_col], errors="coerce").min()
        hi = pd.to_numeric(df[high_col], errors="coerce").max()
        return float(lo), float(hi)
    except Exception:
        return np.nan, np.nan


def fix_regime_trade_log_display_prices_only(trades_df, raw_prices=None):
    """
    DISPLAY ONLY pricing fix.

    Keeps old Regime Switching logic/performance exactly the same.
    It does NOT rerun the strategy on raw prices.

    It only fixes impossible displayed Buy/Sell prices:
    - If a displayed closed-trade price is outside that raw session's High/Low,
      show the raw session close for that date.
    - It stores the original model/smoothed value in Model Buy Price / Model Sell Price.
    - It does NOT recalculate PnL, cumulative return, equity curve, or metrics.

    This solves cases like IREN 06/24 showing 56+ when the session never touched 56+,
    without changing the old strategy behavior that the user liked.
    """
    try:
        ...
        # Keep original model/smoothed values for audit, but don't add unless needed.
        changed_any = False

        def _fix_one(row_idx, date_col, price_col, model_col_name):
            nonlocal changed_any, out
            try:
                if date_col not in out.columns or price_col not in out.columns:
                    return
                dt = out.loc[row_idx, date_col]
                if str(dt).strip().lower() == "open":
                    return
                old_px = pd.to_numeric(out.loc[row_idx, price_col], errors="coerce")
                if pd.isna(old_px) or float(old_px) <= 0:
                    return
                lo, hi = _raw_session_high_low_for_display_date(raw_prices, dt)
                if not np.isfinite(lo) or not np.isfinite(hi) or lo <= 0 or hi <= 0:
                    return
                # Small tolerance for adjusted data rounding.
                tol = 0.0025
                if float(old_px) < lo * (1.0 - tol) or float(old_px) > hi * (1.0 + tol):
                    raw_close = _raw_session_close_for_display_date(raw_prices, dt)
                    if np.isfinite(raw_close) and raw_close > 0:
                        if model_col_name not in out.columns:
                            out[model_col_name] = np.nan
                        out.loc[row_idx, model_col_name] = float(old_px)
                        out.loc[row_idx, price_col] = float(raw_close)
                        changed_any = True
            except Exception:
                pass

        for i in out.index:
            # For OPEN rows, current/latest display can update elsewhere. Only fix entry price here.
            _fix_one(i, "Entry Date", "Buy Price", "Model Buy Price")
            status = str(out.loc[i, "Status"]).lower() if "Status" in out.columns else ""
            if status != "open":
                _fix_one(i, "Exit Date", "Sell Price", "Model Sell Price")

        if changed_any:
            if "Display Price Note" not in out.columns:
                out["Display Price Note"] = ""
            out["Display Price Note"] = out["Display Price Note"].fillna("").astype(str)
            out.loc[:, "Display Price Note"] = out["Display Price Note"].replace("", "Displayed Buy/Sell price adjusted to raw session close when model/smoothed price was outside session range. PnL unchanged.")
...
def build_live_regime_hybrid_signal(anchor_signal, anchor_prices, live_prices, enable_overlay=True, mode="Balanced"):
    """
    Live Regime Hybrid:
    - Uses the stable daily/weekly regime signal as the main brain.
    - Maps that signal onto live intraday candles.
    - Optional intraday trend overlay can enter earlier or protect from clear intraday breakdowns.
    This is causal: it only uses information up to each live candle.
    """
    try:
        live_px = pd.Series(live_prices).replace([np.inf, -np.inf], np.nan).dropna().astype(float)
        if live_px.empty:
            return pd.Series(dtype=float)

        anchor_sig = pd.Series(anchor_signal).replace([np.inf, -np.inf], np.nan).dropna().astype(float).clip(0, 1)
        if anchor_sig.empty:
            base = pd.Series(0.0, index=live_px.index)
        else:
            # Forward-fill the most recent daily/weekly regime decision onto live candles.
            union_idx = anchor_sig.index.union(live_px.index)
            base = anchor_sig.reindex(union_idx).sort_index().ffill().reindex(live_px.index).ffill().fillna(0.0).clip(0, 1)

        if not enable_overlay or len(live_px) < 8:
        ...
        try:
            return pd.Series(anchor_signal).reindex(pd.Series(live_prices).index).ffill().fillna(0.0).clip(0, 1)
        except Exception:
            return pd.Series(dtype=float)
...
with tab7:
    ...
    if strategy_type == "Regime Switching (Trend Following)":
        ...
        # Live Regime Stabilizer: live intraday regime models can be noisy because they
        # only see a short high-frequency window. This keeps the stable daily/weekly
        # regime as the main brain, then maps it to live candles with an optional
        # intraday trend overlay. Non-live behavior is unchanged.
        use_live_regime_hybrid = False
        live_regime_anchor_freq = "Weekly"
        live_regime_overlay = True
        live_regime_sensitivity = "Balanced"
        if live_mode:
            with st.expander("⚡ Live Regime Stabilizer", expanded=True):
                use_live_regime_hybrid = st.checkbox(
                    "Use stable daily/weekly regime brain in live mode",
                    value=True,
                    key="bt_live_regime_hybrid",
                    help="Recommended. Fits Regime Switching on longer daily/weekly history, then maps the signal to live intraday candles."
                )
    ...
    if signals is not None:
        ...
        if strategy_type == "Regime Switching (Trend Following)":
            ...
            with pgc3:
                ...
                if bt_freq == "Daily":
                    st.caption("Daily natural mode: old strategy logic is unchanged. Display-only pricing audit prevents impossible smoothed prices from showing as real session fills.")
                    ...
                    if plot_trades_df is not None and not plot_trades_df.empty:
                        ...
                        try:
                            if strategy_type == "Regime Switching (Trend Following)" and bt_freq == "Daily":
                                ...
                                plot_trades_df = fix_regime_trade_log_display_prices_only(plot_trades_df, raw_prices=df_bt)
                        except Exception:
                            pass
        ...
        if strategy_type == "Regime Switching (Trend Following)" and bt_freq == "Daily":
            st.caption("Daily exact-time button is display-only. It tries to map selected rows to real 5-minute candles. Closed trades keep their historical exit-date price; only open trades may update to the latest/current price.")
        trades_df = bt_results['trades'].copy()
        if not trades_df.empty:
            ...
            # Display-only raw session pricing fix:
            # Keep the old strategy/logic/returns, but stop displaying impossible
            # smoothed model prices as if they were real session fills.
            try:
                if strategy_type == "Regime Switching (Trend Following)" and bt_freq == "Daily":
                    trades_df = fix_regime_trade_log_display_prices_only(trades_df, raw_prices=df_bt)
            except Exception:
                pass

            # FIX: left PnL (%) = single trade return; right Cumulative Return (%)
            # = compounded from those PnL values, oldest-to-newest.
            # If display-only raw price adjustment added Model Buy/Sell columns,
            # keep old model/strategy PnL unchanged. Otherwise recalc as before.
            try:
                if not any(c in trades_df.columns for c in ["Model Buy Price", "Model Sell Price"]):
        ...
        if not _load_micro_tab:
            ...
            if df_main is None:
                ...
                **Included here:**
                ...
                with st.expander("📊 OHLCV Microstructure Proxy Backtest + Strategy Lab", expanded=False):
            ...
            with st.expander("🕒 Databento Historical MBP-1 Replay", expanded=False):
                ...
                if st.button("Pull Databento historical MBP-1 replay", key="mm_full_hist_pull"):
                    if not db_api_key_hist:
                        ...
                        try:
                            start_dt = datetime.combine(replay_date, start_time)
                            end_dt = datetime.combine(replay_date, end_time)
                            with st.spinner(f"Pulling historical MBP-1 for {TICKER}..."):
